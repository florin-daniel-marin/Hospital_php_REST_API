...
@section('content')
    <x-card class="p-5">
        ...
        <div>
            <x-assistant-specs :specsCsv="$assistant->spec"  assistant/> 
        </div>

        <div class="w-50">
        <figcaption class="ms-4">
            <strong>Cabinet {{$assistant->room}} <br> {{$assistant->sex}} <br> </strong>
            <span class="text-muted">{{$assistant->phone}} <br> {{$assistant->email}} <br> </span>
        </figcaption>
        </div>

        <div class="ms-4 h6">
            <p class="h6"> Address: p-3>{{$assistant->address}} 
                <br> Date of birth: {{$assistant->dob}} 
                <br> Has account: {{$assistant->hacAccount}}</p>
        </div>
    </x-card>
