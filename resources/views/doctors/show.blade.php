...
@section('content')
    <x-card class="p-5">
        ...
        <div>
            <x-doctor-specs :specsCsv="$doctor->spec"  doctor/> 
        </div>

        <div class="w-50">
        <figcaption class="ms-4">
            <strong>Cabinet {{$doctor->room}} <br> {{$doctor->sex}} <br> </strong>
            <span class="text-muted">{{$doctor->phone}} <br> {{$doctor->email}} <br> </span>
        </figcaption>
        </div>

        <div class="ms-4 h6">
            <p class="h6"> Address:>{{$doctor->address}} 
                <br> Date of birth: {{$doctor->dob}} 
                <br> Has account: {{$doctor->hacAccount}}</p>
        </div>
    </x-card>
